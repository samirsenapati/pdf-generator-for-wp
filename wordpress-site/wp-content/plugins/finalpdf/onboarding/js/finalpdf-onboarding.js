jQuery(document).ready(function($) {
    ...
    if ('plugins.php' == wps_pgfw_onboarding.pgfw_current_screen) {
        ...
        jQuery(document).on('change', '.finalpdf-on-boarding-radio-field', function(e) {

            e.preventDefault();
            if ('other' == jQuery(this).attr('id')) {
                jQuery('#deactivation-reason-text').removeClass('wps-finalpdf-keep-hidden');
            } else {
                jQuery('#deactivation-reason-text').addClass('wps-finalpdf-keep-hidden');
            }
        });
    } else {
        // Show Popup after 1 second of entering into the MWB pagescreen.
        if (jQuery('#wps-finalpdf-show-counter').length > 0 && jQuery('#wps-finalpdf-show-counter').val() == 'not-sent') {
            setTimeout(wps_pgfw_show_onboard_popup(), 1000);
        }
    }

    /* Close Button Click */
    jQuery(document).on('click', '.wps-finalpdf-on-boarding-close-btn a', function(e) {
        e.preventDefault();
    ...
    /* Skip For a day. */
    jQuery(document).on('click', '.wps-finalpdf-on-boarding-no_thanks', function(e) {
    ...
    /* Submitting Form */
    jQuery(document).on('submit', 'form.wps-finalpdf-on-boarding-form', function(e) {

        e.preventDefault();
        var form_data = JSON.stringify(jQuery('form.wps-finalpdf-on-boarding-form').serializeArray());
    ...
    function wps_pgfw_add_deactivate_slugs_callback(all_slugs) {
        ...
        for (var i = all_slugs.length - 1; i >= 0; i--) {
            ...
            jQuery(document).on('click', '#deactivate-' + all_slugs[i], function(e) {
                ...
                jQuery('.wps-finalpdf-on-boarding-heading').text(plugin_name + ' Feedback');
                var placeholder = jQuery('#wps-finalpdf-deactivation-reason-text').attr('placeholder');
                jQuery('#wps-finalpdf-deactivation-reason-text').attr('placeholder', placeholder.replace('{plugin-name}', plugin_name));
