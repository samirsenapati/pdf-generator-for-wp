		...
		if ( is_object( $post ) ) {
			...
			if ( 'yes' == $finalpdf_template_color_option ) {
				...
				<div class="finalpdf-pdf-body">
					<div class="finalpdf-pdf-body-title-image">
						<img src="' . get_the_post_thumbnail_url( $post ) . '">
					</div>
					<div class="finalpdf-pdf-body-title">
						' . do_shortcode( str_replace( '[WORDPRESS_PDF]', '', apply_filters( 'the_title', $post->post_title, $post->ID ) ) ) . '
					</div>
					<div class="finalpdf-pdf-body-content">
					<h3>' . esc_html__( 'Description', 'finalpdf' ) . '</h3>
					<div>
					
						' . do_shortcode( str_replace( '[WORDPRESS_PDF]', '', apply_filters( 'the_content', apply_filters( 'wps_finalpdf_customize_template_post_content', $post->post_content, $post ) ) ) ) . '
				</div>';
